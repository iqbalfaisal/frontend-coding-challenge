...
export default function Stepper({ currentStep, steps }: StepperProps) {
	const renderStep = (step: Step, index: number) => {
		...
		return <li key={index + step.title} aria-current={isActive ? "step" : undefined} className="flex-1 flex flex-shrink-0 flex-col items-center relative px-5">
			{!isLast && (
				<div className={`absolute top-1/2 left-1/2 w-full h-[4px] sm:h-[6px] transform -translate-y-2 sm:-translate-y-3 -z-10 
                            ${index < currentStep ? "bg-blue-500" : "bg-gray-200"}`}>
				</div>
			)}
			<div className={`size-[30px] sm:size-[40px] flex items-center mt-3 text-[12px] leading-[16px] sm:text-[14px] sm:leading-[20px] justify-center rounded-full transition-colors duration-300
